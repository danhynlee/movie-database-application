...
begin
        ...
        SELECT i_creditCardNum, i_username
		FROM movie
		WHERE NOT EXISTS (SELECT 1
        FROM movieplay
        WHERE creditCardNum = i_creditCardNum AND username = i_username);
end$$
delimiter ;
...
begin
    ...
    select user.username as username, count(creditCardNum) as creditCardCount, (case
	...
	(CASE WHEN i_sortBy = ""  or i_sortBy is NULL AND i_sortDirection = "" or i_sortDirection is NULL THEN user.username END) DESC;
end $$
delimiter ;
...
begin
	...
	(CASE WHEN i_sortBy = "" or i_sortBy is Null AND i_sortDirection = "" or i_sortDirection is NULL 
    THEN thdata.comName END) DESC;
end$$
delimiter ;
...
BEGIN
        ...
        SELECT i_thName, i_comName, i_thStreet, i_thCity, i_thState, i_thZipcode, i_capacity, i_manUsername
		FROM theater
		WHERE NOT EXISTS (SELECT 1
        FROM theater
        WHERE thName = i_thName AND comName = i_comName AND thStreet = i_thStreet AND thCity = i_thCity AND thState = i_thState AND thZipcode = i_thZipcode AND manUsername = i_manUsername);
END$$
DELIMITER ;
...
BEGIN
        ...
        SELECT i_movName, i_movReleaseDate, i_movDuration
		FROM movie
		WHERE NOT EXISTS (SELECT 1
        FROM movieplay
        WHERE movName = i_movName AND i_movReleaseDate = movReleaseDate AND duration = i_movDuration);
END$$
DELIMITER ;
...
BEGIN
        ...
        NOT EXISTS (SELECT 1
        FROM movieplay
        WHERE movPlayDate = i_movPlayDate AND movName = i_movName AND i_movReleaseDate = movReleaseDate AND theater.thName = movieplay.thName);
END$$
DELIMITER ;
...
BEGIN
    ...
    WHERE
        ...
        (thState = i_state OR i_state ="" or i_state = "ALL") AND
        (i_minMovPlayDate IS NULL OR movPlayDate >= i_minMovPlayDate) AND
